...
function createNewGroup(){
  const createGroupForm = document.getElementById('group');
...
document.getElementById('create-group-submit-btn').addEventListener('click', createNewGroup);
...
function searchAndCreateAddUserUI(target){
    ...
    // got to database
target.parentElement.parentElement.querySelector('.searchResults').innerHTML = `<div class="findOne">Add User</div><button type="button" class="addUser-btn" onclick="addUserToGroup(this);">${searchParam}<span class="plus"><i class="fa fa-plus"></i></span></button>`;
	form.reset();
}
